...
- **Roadmap ref:** `specs/roadmap.md`, Iteration 2, row xx (renumbered with the upstream issue). Extends Iteration 1 feature #8 (app-only `roles`) and the token-configuration work (issue #15, migration 002) to **delegated** tokens. Related upstream discussion: issue #30 / PR #31 (auto-granted `roles` on delegated access tokens).
